...
extern uint8_t temp_buf[8];
extern uint32_t temperature[2];
...
void ind_pwm_init()
{
	...
	TIM3->ARR = 2000-1 ;    // TIM3 period 100 Hz
	TIM3->CCR3 = 1850;
	TIM3->CCR4 = 2000;
...
const uint32_t temp_sens_addr[2] = {0x90, 0x86};
...
void TIM3_IRQHandler(void)
{
	...
	//i2c_start(0x90);
	//i2c_start(0x86);
	temperature[ti] = temp_buf[0];
	ti ^= 1;
	i2c_start(temp_sens_addr[ti]);
