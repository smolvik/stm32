		...
		var mixin = {
			...
			_getBaseUrl: function () {
				...
				if (w.begun_urls && w.begun_urls.base_scripts_url) {
					...
					var adsDomain = w.begun_block_id ? 'begun' : w.price_block_id ? 'price' : 'begun';
					url = 'http://autocontext.' + adsDomain + '.ru/';
				}
			...
			_norm: function(scriptUrl, isStrict) {
				if (null != scriptUrl) {
					if (
						scriptUrl.indexOf(this._getBaseUrl()) !== 0
							&& !urlUtils.hasDomain(scriptUrl)
					) {
						scriptUrl = this._getBaseUrl() + scriptUrl.replace(/\?.*$/, '');
					}
